...
export function quatRotate(v,q){const t=cross({x:q.x,y:q.y,z:q.z},v),u={x:2*t.x,y:2*t.y,z:2*t.z},c=cross({x:q.x,y:q.y,z:q.z},u);return{x:v.x+q.w*u.x+c.x,y:v.y+q.w*u.y+c.y,z:v.z+q.w*u.z}}
...
export function formatYear(y){const n=Math.round(y);if(n>10000)return`+${(n/1000000).toFixed(n%1000000?2:0)} Ma · future`;if(n<0)return`${Math.abs(n).toLocaleString()} BCE`;return`${Math.max(1,n).toLocaleString()} CE`}
export function epochForYear(y){if(y>10000)return'Future supercontinent scenario · Pangea Ultima';if(y>0)return'Holocene · present Earth';if(y>-11700)return'Late Pleistocene / Holocene';if(y>-2600000)return'Pleistocene';if(y>-66000000)return'Cenozoic';if(y>-145000000)return'Late Jurassic / Early Cretaceous';if(y>-252000000)return'Mesozoic';if(y>-335000000)return'Late Paleozoic';if(y>-541000000)return'Paleozoic';return'Neoproterozoic · Pannotia era'}
