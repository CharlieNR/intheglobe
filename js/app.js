...
let renderer;
try{renderer=new THREE.WebGLRenderer({canvas,antialias:true,powerPreference:'high-performance'});renderer.setPixelRatio(Math.min(window.devicePixelRatio||1,2));renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.toneMapping=THREE.NoToneMapping}catch(error){logError(error);throw error}
...
const paleoOcean=new THREE.Mesh(new THREE.SphereGeometry(.998,96,72),new THREE.MeshBasicMaterial({color:0x176d9a}));paleoOcean.visible=false;earthGroup.add(paleoOcean);
