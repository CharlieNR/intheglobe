...
function shiftedHoleRing(ring,anchor){const unwrapped=unwrapRing(ring);if(!unwrapped.length)return[];const shift=360*Math.round((anchor-unwrapped[0][0])/360);return unwrapped.map(([lon,lat])=>[lon+shift,lat])}
...
function cleanRing(ring){const out=[];for(const point of ring||[]){const lon=Number(point[0]),lat=Number(point[1]);if(!Number.isFinite(lon)||!Number.isFinite(lat))continue;if(!out.length||Math.abs(lon-out[out.length-1][0])>1e-10||Math.abs(lat-out[out.length-1][1])>1e-10)out.push([lon,lat])}if(out.length>1&&Math.abs(out[0][0]-out[out.length-1][0])<1e-10&&Math.abs(out[0][1]-out[out.length-1][1])<1e-10)out.pop();return out}
function landMesh(data,radius=1.002,material){const group=new THREE.Group();const mat=material||new THREE.MeshBasicMaterial({color:0x9eb48d,side:THREE.DoubleSide});const positions=[];for(const rings of polygonRings(data)){const outer=cleanRing(unwrapRing(rings[0]));if(outer.length<3)continue;const anchor=outer[0][0];const holes=rings.slice(1).map(r=>cleanRing(shiftedHoleRing(r,anchor))).filter(r=>r.length>=3);const contour=outer.map(([lon,lat])=>new THREE.Vector2(lon,lat));const holePoints=holes.map(r=>r.map(([lon,lat])=>new THREE.Vector2(lon,lat)));const tris=THREE.ShapeUtils.triangulateShape(contour,holePoints);if(!tris.length)continue;const all=[outer,...holes].flat();for(const tri of tris)for(const idx of tri){const point=all[idx];if(!point)continue;const [lon,lat]=point;const p=geoPoint(lon,lat,radius);positions.push(p.x,p.y,p.z)}}if(!positions.length)return group;const geometryBuffer=new THREE.BufferGeometry();geometryBuffer.setAttribute('position',new THREE.Float32BufferAttribute(positions,3));geometryBuffer.computeBoundingSphere();const mesh=new THREE.Mesh(geometryBuffer,mat);mesh.renderOrder=10;group.add(mesh);return group}

// Modern country geometry is rendered as an equirectangular canvas texture rather than
// triangulating lon/lat polygons directly. This avoids the large false triangles and
// holes that can occur around complex countries and the antimeridian (notably the US/Russia).
function makeGeoTexture(data,{fill='rgba(22,140,255,0.24)',stroke='rgba(70,183,255,0.90)',lineWidth=3,size=4096}={}){
  ...
  ctx.fillStyle=fill;ctx.strokeStyle=stroke;ctx.lineWidth=lineWidth;ctx.lineJoin='round';ctx.lineCap='round';
...
export async function loadModernCountryBorders(radius=1.006){const countries=await loadModernCountries();return textureGlobe(countries,radius,{fill:'rgba(0,0,0,0)',stroke:'rgba(70,183,255,0.90)',lineWidth:2.5,size:4096})}
export async function loadModernCountryOverlay(radius=1.012){const countries=await loadModernCountries();return textureGlobe(countries,radius,{fill:'rgba(22,140,255,0.24)',stroke:null,lineWidth:0,size:4096})}

// Keep only a small number of paleo JSON frames in live JS memory. The timeline can
// still be fully prefetched into IndexedDB, but retaining every 50/5/2.5 Ma frame in
// a Map can consume hundreds of MB (or more) and eventually kill the WebGL context.
const paleoCache=new Map();
const MAX_LIVE_PALEO_DATA=4;
function rememberPaleoData(key,value){paleoCache.delete(key);paleoCache.set(key,value);while(paleoCache.size>MAX_LIVE_PALEO_DATA){const oldest=paleoCache.keys().next().value;if(oldest===key)break;paleoCache.delete(oldest)}}
async function getPaleoData(year,model='CAO2024'){
  ...
  const url=`${GWS}coastlines/?time=${encodeURIComponent(age)}&model=${encodeURIComponent(model)}`;
  const r=await fetch(url,{cache:'no-store'});if(!r.ok)throw new Error(`GPlates HTTP ${r.status}`);
  const data=await r.json();await persistentPut(key,data);rememberPaleoData(key,data);return data;
}
export async function loadPaleoCoastlines(year,model='CAO2024',radius=1.008){const data=await getPaleoData(year,model);const group=new THREE.Group();const material=new THREE.LineBasicMaterial({color:0xd9f3e6,transparent:true,opacity:.9,depthWrite:false});addGeometryLines(group,data,radius,material);return group}
export async function loadPaleoLand(year,model='CAO2024',radius=1.002){const data=await getPaleoData(year,model);return landMesh(data,radius,new THREE.MeshBasicMaterial({color:0x9eb48d,side:THREE.DoubleSide}))}
...
export async function prefetchReconstructedCountryFrames(countryFeature,agesMa,model='CAO2024',concurrency=12,onProgress){const ages=[...new Set(agesMa.map(Number).filter(Number.isFinite).map(a=>Math.max(0,Math.min(600,a))))].sort((a,b)=>a-b);const parts=countryParts(countryFeature);let done=0,cursor=0;const worker=async()=>{while(true){const i=cursor++;if(i>=ages.length)return;try{await Promise.all(parts.map(part=>reconstructFeatureCollection({type:'FeatureCollection',features:[part]},ages[i],model)));done++}catch{}finally{onProgress?.(done/ages.length,done,ages.length)}}};await Promise.all(Array.from({length:Math.min(concurrency,ages.length)},worker));return done===ages.length}
export function makeCountryHighlight(data,radius=1.012){return textureGlobe(data,radius,{fill:'rgba(255,20,147,0.42)',stroke:'rgba(255,20,147,1)',lineWidth:3,size:4096})}
